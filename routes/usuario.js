...
router.post('/registro', (req, res) => {
  ...
  if(erros.length > 0){
    ...
    Usuario.findOne({email: req.body.email}).then((usuario)=>{
      if(usuario){
        ...
        bcrypt.genSalt(10, (erro, salt) => {
          bcrypt.hash(novoUsuario.senha, salt, (erro, hash) => {
            if(erro){
              req.flash('error_msg', 'houve um erro de salvamento do usuario')
              res.redirect('/')
            }
            ...
            novoUsuario.save().then(() => {
              ...
              req.flash('error_msg', 'Houve um erro ao salvar a senha do usuario')
              res.redirect('/usuarios/registro')
            })
          })
      ...
      req.flash('error_msg', 'houve um erro de encrypt interno')
      res.redirect('/')
    })
  }
